...
if (isset($_POST['tag']) && $_POST['tag'] != '') {
    ...
    if ($tag == 'forgot_pass') {
        ...
        if ($row) {
            $stmt = $db->prepare("UPDATE haritkandharapp_user SET encrypted_password = :newPass WHERE email = :email");
    ...
    // Change Password
        ...
        if ($row) {
            ...
            if (password_verify($oldPassword, $row['encrypted_password'])) {
                $stmt = $db->prepare("UPDATE haritkandharapp_user SET encrypted_password = :password WHERE email = :email");
